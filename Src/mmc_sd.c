...
//	SSP_InitTypeDef SSP1_InitStructure;
	...
	// if (SD_Idle_Sta()) return 1; 	// 1 set out to return to the idle mode failed
	...
	else if (r1 == 0x01)
	{
	    ...
	    buff[0] = SD_IO_ReadByte (); // should be 0x00
	    buff[1] = SD_IO_ReadByte (); // should be 0x00
	    buff[2] = SD_IO_ReadByte (); // should be 0x01
	    buff[3] = SD_IO_ReadByte (); // should be 0xAA
	    
	    MSD_CS_DISABLE();
	    SD_IO_WriteByte (0xFF); // the next 8 clocks
...
uint8_t SD_GetResponse (uint8_t Response)
{
	...
	while ((SD_IO_ReadByte () != Response) && Count) 
...
uint8_t SD_WaitDataReady (void)
{
	...
	do
	{
       	 r1 = SD_IO_ReadByte () &0X1F; // read response
	...
	retry = 0;
	while (SD_IO_ReadByte () == 0) // read data to 0, the data has not yet completed writing
	{
		retry++;
...
uint8_t SD_SendCommand (uint8_t cmd, uint32_t arg, uint8_t crc)
{
	...
	SD_IO_WriteByte (0xff); // high-speed write command delay
	SD_IO_WriteByte (0xff);
	SD_IO_WriteByte (0xff);
	...
	// Send
	SD_IO_WriteByte (cmd | 0x40); // write command, respectively,
	SD_IO_WriteByte (arg>> 24);
	SD_IO_WriteByte (arg>> 16);
	SD_IO_WriteByte (arg>> 8);
	SD_IO_WriteByte (arg);
	SD_IO_WriteByte (crc);
	
	// Wait for a response, or time-out exit
	while ((r1 = SD_IO_ReadByte ()) == 0xFF)
	{
		Retry++;
		if (Retry> SD_MEMORY_RETRY_COUNT) 
			break;
...
uint8_t SD_SendCommand_NoDeassert (uint8_t cmd, uint32_t arg, uint8_t crc)
{
	...
	// Wait for a response, or time-out exi
	while ((r1 = SD_IO_ReadByte ()) == 0xFF)
	{
	    Retry++;
	    if (Retry> SD_MEMORY_RETRY_COUNT) 
...
uint8_t SD_Idle_Sta (void)
{
	...
	for (i = 0; i <10; i++) SPIx_Write(0xFF);
...
uint8_t MSD_WriteBuffer (uint8_t * pBuffer, uint32_t WriteAddr, uint32_t NumByteToWrite)
{
	...
	while (NbrOfBlock--) // write a sector
	{
		...
		SD_IO_WriteByte (0xFE); // put initial token 0xFE
		
		// Put data in a sector
		for (i = 0; i <512; i++) 
			SD_IO_WriteByte (*pBuffer++);
...
uint8_t SD_WriteSingleBlock (uint32_t sector, const uint8_t * data)
{
	...
	SD_IO_WriteByte (0xFE);

	// Put data in a sector
	for (i = 0; i <512; i++)
	{
		SD_IO_WriteByte (* data++);
	}
...
uint8_t SD_WriteMultiBlock (uint32_t sector, const uint8_t * data, uint8_t count)
{
	...
	SD_IO_WriteByte (0xff); // first put three empty data, SD card ready to wait
	SD_IO_WriteByte (0xff);
	...
	// r1 = SD_IO_ReadByte (0xFD);
	SD_IO_WriteByte (0xFD);
	r1 = SD_IO_ReadByte();
...
uint8_t SD_Read_Bytes (unsigned long address, unsigned char * buf, unsigned int offset, unsigned int bytes)
{
	...
	for (i = 0; i <offset; i++) SD_IO_WriteByte (0xff); // skip the offset bits
	for (; i <offset + bytes; i++) * buf++= SD_IO_ReadByte (); // read the useful data
	for (; i <512; i++) SD_IO_WriteByte (0xff); // read the remaining bytes
	
	SD_IO_WriteByte (0xff); // send the pseudo-code CRC
	SD_IO_WriteByte (0xff);
